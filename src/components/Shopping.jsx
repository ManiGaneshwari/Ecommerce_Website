...
const Shopping = () => {
  ...
  const carouselImages = [banner3, banner2, banner1];
  ...
  const filteredProducts = apiData?.filter((item) => {
    ...
    const matchesCategory = selectedCategory === "" || item?.category === selectedCategory.toLowerCase();
        ...
        {filteredProducts.map((data) => (
          <div key={data?.id} className="product-card">
            <div className="image-wrapper">
              <img src={data?.image} alt={`Product: ${data?.title}`} loading="lazy" />
            </div>
            <div>
